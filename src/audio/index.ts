...
function initGraph() {
  ...
  (Object.keys(assets) as SfxName[]).forEach((name: SfxName)=>{
...
function start() {
  ...
  ctx!.resume().then(()=>{
    started = true;
    (['engine_idle','engine_drive'] as SfxName[]).forEach((n: SfxName)=>{ if (media[n]) { media[n]!.currentTime = 0; media[n]!.play().catch(()=>{}); } })
  })
}
